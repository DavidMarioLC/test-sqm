...
export const ArticleFilters = ({ country, language }) => {
  ...
  React.useEffect(() => {
    const subscription = form.watch((value, { name, type }) => {
      if (value.search || value.application || value.nutrients || value.crops) {
        setCleanButton(true);
      } else {
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)}>
        <label className="font-montserrat text-base font-normal">
          {translations.filters.articleFilterLabel}
        </label>
                ...
                <FormItem className="flex-1">
                  <FormControl>
                    <Select value={field.value} onValueChange={field.onChange}>
                      <SelectTrigger>
                        <SelectValue
                          placeholder={translations.common.selectNutrient}
                        />
                      </SelectTrigger>
                      <SelectContent>
                        {nutrients.map((item) => (
